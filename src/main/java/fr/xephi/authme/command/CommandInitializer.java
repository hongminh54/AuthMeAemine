...
import fr.xephi.authme.command.executable.authme.ResetIpCommand;
...
public class CommandInitializer {
    ...
    private CommandDescription buildAuthMeBaseCommand() {
        ...
        // Register the resetip command
        CommandDescription.builder()
            .parent(authmeBase)
            .labels("resetip", "clearip", "ipreset")
            .description("Reset IP cache")
            .detailedDescription("Reset IP cache and restrictions for a specific player/IP or clear all cache.")
            .withArgument("player/ip", "Player name, IP address, or empty for all", OPTIONAL)
            .permission(AdminPermission.RESET_IP)
            .executableCommand(ResetIpCommand.class)
            .register();
